...
// Add Zen Tabs styles
if (theme_get_setting('iied_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'basic') .'/css/tabs.css');
}

function iied_preprocess_page(&$vars, $hook) {
...
function iied_preprocess_node(&$vars) {
  // Add a striping class.
  $vars['classes_array'][] = 'node-' . $vars['zebra'];

  // Merge first/last class (from iied_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function iied_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function iied_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('iied_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('iied_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('iied_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('iied_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('iied_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 * 
 */
function iied_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = iied_id_safe($element['#title']);
 ...
 */
function iied_preprocess_menu_local_task(&$variables) {
 ...
 */
function iied_menu_local_tasks(&$variables) {  
  $output = '';
 ...
 */
function iied_node_view_alter(&$build) {
 ...
 */
function iied_menu_contextual_links_alter(&$links, $router_item, $root_path) {
  if ($root_path == 'node/%') {
